...
class ConcLandTileManager:
    ...
    def draw_tile(self, tile_id: str, x: int, y: int, transparent_color: int = 0):
        ...
        # Debug: Print drawing info for first few calls
        ...
        if self._debug_count <= 5:
            print(f"Drawing tile '{tile_id}' at screen({x},{y}) from tilemap({tile_pos[0]},{tile_pos[1]}) size={self.tile_size}")
            
        pyxel.blt(x, y, self.image_bank, tile_pos[0], tile_pos[1], 
                  self.tile_size, self.tile_size, transparent_color)
